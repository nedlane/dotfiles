...
: > "$log"
out="$(run addguest ghpr 222222222222222222 view)" || fail "addguest failed: $out"
...
: > "$log"
run addguest ghpr 222222222222222222 >/dev/null || fail "addguest default-access failed"
...
: > "$log"
run request 222222222222222222 photo-pipeline "wants to help" >/dev/null || fail "request failed"
...
: > "$log"
run revoke ghpr 222222222222222222 >/dev/null || fail "revoke failed"
grep -q '"claude.bridge.revoke"' "$log" || fail "no revoke event: $(cat "$log")"
grep -q '"discord_id": "222222222222222222"' "$log" || fail "revoke id missing: $(cat "$log")"
