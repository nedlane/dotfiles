# Generated by Powerlevel10k configuration wizard on 2023-09-03 at 14:44 AEST.
...
() {
  ...
  # Simplified left prompt: hostname (prepended as `context` by the dotfiles
  # zshrc), then OS icon, then the current directory. Git/vcs lives on the
  # right now.
  ...
  # Simplified right prompt. Most of these only render when relevant (a git
  # repo, an active venv, a kube/aws/terraform context, ...) so day-to-day the
  # right side is mostly just the time, with public IP (and battery on the Mac)
  # on the second line. Battery is appended per-host by the dotfiles zshrc.
